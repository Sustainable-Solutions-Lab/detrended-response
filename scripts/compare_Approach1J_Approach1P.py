#!/usr/bin/env python3
"""Compare parameters from Approach1J (conjoined OLS) vs Approach1P (precomputed k).
...
Each panel shows Approach1P predictions on x-axis vs Approach1J values on y-axis,
with a 1:1 reference line and R^2 annotation.

For the j coefficients, Approach1P's raw predictions are re-referenced by subtracting
country 0's values (to match Approach1J's identification constraint j=0 for country 0).
The subtracted quadratic is absorbed into k, so Approach1P's k becomes:
  k'(t) = k_mean(t) + j_{0,0} + j_{1,0}*t + j_{2,0}*t^2

Outputs:
  - Approach1J_vs_Approach1P_scatter.pdf: 2x2 scatter plot figure
  - Approach1J_vs_Approach1P_scatter_data.csv: raw scatter data
  - Approach1J_vs_Approach1P_table.tex: LaTeX table with slope/intercept/R²/r
"""
...
def compute_predicted_j(data, trends_with_k, trends, h1, h2):
    """Compute predicted j coefficients from Approach1P trends and given h1, h2.
    ...
    Then re-referenced by subtracting country 0's values (to match Approach1J's
...
def make_scatter_panel(ax, x, y, label, panel_letter):
    ...
    ax.set_xlabel(f'Approach1P ({label})', fontsize=9)
    ax.set_ylabel(f'Approach1J ({label})', fontsize=9)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare Approach1J vs Approach1P parameters via scatter plots."
    )
    parser.add_argument(
        "--data-file", type=str,
    ...
    fig.suptitle('Approach1J vs Approach1P: Parameter Comparison',
    ...
    latex_lines.extend([
        r"\bottomrule",
        r"\end{tabular}",
        r"\caption{Best-fit regression of Approach1J values (y-axis) vs Approach1P predictions (x-axis). "
        ...
        r"\label{tab:Approach1J_vs_Approach1P}",
        r"\end{table}",
    ])
    latex_table = '\n'.join(latex_lines)

    # Save LaTeX table
    tex_path = output_dir / "Approach1J_vs_Approach1P_table.tex"
